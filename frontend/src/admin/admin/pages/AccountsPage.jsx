...
const matchText = (account, keyword) =>
  [account.Username, account.FullName, account.Email, account.Phone]
    .join(" ")
    .toLowerCase()
...
export default function AccountsPage() {
  ...
  const filteredAccounts = accounts.filter((account) => {
    const sameRole = !role || account.Role === role;
    const sameText = !search || matchText(account, search);
  ...
  const changeStatus = (account) => {
    apiSend(`/api/admin/accounts/${account.Id}/status`, "PUT", {
      Role: account.Role,
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-col gap-3 rounded-lg bg-white p-4 shadow-sm md:flex-row">
        ...
        <select value={role} onChange={(e) => updateRole(e.target.value)} className="rounded border px-3 py-2">
            ...
            {pageItems.map((account) => {
              ...
              return (
                <tr key={`${account.Role}-${account.Id}`} className="border-t">
                  ...
                  <td className="p-3">
                    <button onClick={() => setSelected(account)} className="mr-2 rounded bg-gray-100 p-2 text-gray-700">
                      <Eye className="h-4 w-4" />
                    </button>
                    <button
                      onClick={() => changeStatus(account)}
                      className={`rounded p-2 ${locked ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"}`}
                    >
